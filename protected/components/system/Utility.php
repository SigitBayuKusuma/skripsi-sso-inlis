...
class Utility
{
	 ...
	 * 10 = Install
	 */
	public static function getPublish($url, $id, $type=null) 
	{
		$arrType = explode(',', $type);
		if($type != null && count($arrType) > 1 ) {
			$plus = $arrType[0];
			$min = $arrType[1];
		} else {
	...
	public static function dateFormat($date, $time=false) {		
		if($time == true)
			$date = date(Yii::app()->params['site_dateformat'], strtotime($date)).' '.date(Yii::app()->params['site_timeformat'], strtotime($date)).' WIB';
		else
			$date = date(Yii::app()->params['site_dateformat'], strtotime($date));
